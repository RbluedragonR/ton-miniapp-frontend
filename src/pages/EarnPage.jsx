...
import {
    ...
    initiateOXYBLEUnstake,
    confirmOXYBLEUnstake
} from '../services/api';
import {
    ...
    toOXYBLESmallestUnits as convertToOXYBLESmallestUnits,
...
const EarnPage = () => {
    ...
    const [calculatedOXYBLEAmount, setCalculatedOXYBLEAmount] = useState(0);

    const [currentArxPrice, setCurrentArxPrice] = useState(null);
    const [OXYBLEWalletBalance, setOXYBLEWalletBalance] = useState(0);
    const [userStakesData, setUserStakesData] = useState({ stakes: [], totalClaimableUsdt: '0.00', totalClaimableOXYBLE: '0.00' });
    ...
    const OXYBLE_MASTER_FROM_CONFIG = stakingConfigData?.arxToken?.masterAddress || import.meta.env.VITE_OXYBLE_TOKEN_MASTER_ADDRESS;
    ...
    const fetchInitialData = useCallback(async (showMessages = false) => {
        ...
        if (showMessages) message.loading({ content: 'Loading OXYBLE Earn Hub...', key: loadingKey, duration: 0 });
        ...
        try {
            ...
            if (rawAddress) {
                ...
                setUserStakesData({
                    ...
                    totalClaimableOXYBLE: stakesResponse.data?.totalClaimableOXYBLE || '0.00'
                });

                if (OXYBLE_MASTER_FROM_CONFIG) {
                    const userOXYBLEJW = await getJettonWalletAddress(rawAddress, OXYBLE_MASTER_FROM_CONFIG);
                    if (userOXYBLEJW) {
                        const balanceNano = await getJettonBalance(userOXYBLEJW);
                        setOXYBLEWalletBalance(fromOXYBLESmallestUnits(balanceNano));
                    } else {
                ...
                setOXYBLEWalletBalance(0);
                setUserStakesData({ stakes: [], totalClaimableUsdt: '0.00', totalClaimableOXYBLE: '0.00' });
            }
    ...
    }, [rawAddress, OXYBLE_MASTER_FROM_CONFIG, currentArxPrice]);
    ...
    useEffect(() => {
        if (inputUsdtAmount && currentArxPrice && currentArxPrice > 0) {
            setCalculatedOXYBLEAmount(parseFloat((inputUsdtAmount / currentArxPrice).toFixed(OXYBLE_DECIMALS)));
        } else {
            setCalculatedOXYBLEAmount(0);
        }
    }, [inputUsdtAmount, currentArxPrice]);

    const handlePlanSelect = (plan) => {
        if (!userFriendlyAddress) {
            message.warn("Please connect your wallet to stake OXYBLE.");
            tonConnectUI.openModal();
            return;
        }
        if (currentArxPrice === null || currentArxPrice <= 0) {
            message.error("OXYBLE price is currently unavailable. Please refresh.", 3);
    ...
    const handleConfirmStake = async () => {
        if (!rawAddress || !selectedPlan || !inputUsdtAmount || inputUsdtAmount <= 0 || calculatedOXYBLEAmount <= 0) {
        ...
        if (calculatedOXYBLEAmount > OXYBLEWalletBalance) {
            message.error(`Insufficient OXYBLE balance. You need ${calculatedOXYBLEAmount.toFixed(OXYBLE_DECIMALS)} OXYBLE, but have ${OXYBLEWalletBalance.toFixed(OXYBLE_DECIMALS)} OXYBLE.`, 4);
            return;
        }

        setStakeSubmitLoading(true);
        const loadingMessageKey = 'stakeActionModalEarnPage';
        message.loading({ content: 'Preparing OXYBLE stake transaction...', key: loadingMessageKey, duration: 0 });

        const dbStakeUUID = uuidv4();
        const scStakeIdentifier = BigInt('0x' + dbStakeUUID.replace(/-/g, '').substring(0, 16));

        try {
            const userOXYBLEJettonWallet = await getJettonWalletAddress(rawAddress, OXYBLE_MASTER_FROM_CONFIG);
            if (!userOXYBLEJettonWallet) throw new Error("Your OXYBLE Jetton Wallet could not be found. Ensure you have OXYBLE.");

            const amountInSmallestOXYBLEUnits = convertToOXYBLESmallestUnits(calculatedOXYBLEAmount);
            const scPayloadParams = {
                queryId: BigInt(Date.now()),
                stakeIdentifier: scStakeIdentifier,
                durationSeconds: parseInt(selectedPlan.durationDays, 10) * 24 * 60 * 60,
                OXYBLE_lock_apr_bps: 0,
                OXYBLE_lock_penalty_bps: parseInt(parseFloat(selectedPlan.OXYBLEEarlyUnstakePenaltyPercent) * 100 || 0),
            };
            const forwardPayloadCell = createStakeForwardPayload(scPayloadParams);
            const jettonTransferBody = createJettonTransferMessage(
                amountInSmallestOXYBLEUnits, STAKING_CONTRACT_JW_FROM_CONFIG, rawAddress,
                toNano("0.1"), forwardPayloadCell
            );
            const transaction = {
                validUntil: Math.floor(Date.now() / 1000) + 360,
                messages: [{
                    address: userOXYBLEJettonWallet,
            ...
            if (!txHash) {
                throw new Error('Failed to confirm stake transaction on blockchain. Please check your wallet activity. If OXYBLE was deducted without stake appearing, contact support.');
            }

            message.loading({ content: 'Transaction confirmed! Recording OXYBLE stake with backend...', key: loadingMessageKey, duration: 0 });
            const referrerCode = localStorage.getItem('OXYBLEReferralCode');
            await recordUserStake({
                planKey: selectedPlan.key || selectedPlan.id.toString(),
                OXYBLEAmount: calculatedOXYBLEAmount,
            ...
            message.success({ content: `Successfully staked ${calculatedOXYBLEAmount.toFixed(OXYBLE_DECIMALS)} OXYBLE! USDT rewards will now accrue.`, key: loadingMessageKey, duration: 6 });

            setIsStakeModalVisible(false); setSelectedPlan(null); setInputUsdtAmount(null);
            if (referrerCode) localStorage.removeItem('OXYBLEReferralCode');
            setTimeout(() => { fetchInitialData(false); }, 8000);
            setTimeout(() => { fetchInitialData(false); }, 40000);
        } catch (error) {
            message.error({ content: error?.response?.data?.message || error?.message || 'OXYBLE Staking failed. Please try again.', key: loadingMessageKey, duration: 6 });
            console.error('[EarnPage] OXYBLE Staking tx/record error:', error);
        } finally {
    ...
    const handleUnstakeActionClick = (stake) => {
        ...
        initiateOXYBLEUnstake({ userWalletAddress: rawAddress, stakeId: stake.id })
            .then(response => {
    ...
    const handleConfirmUnstakeInModal = async () => {
        ...
        message.loading({ content: 'Please confirm OXYBLE unstake in your wallet...', key: loadingMessageKey, duration: 0 });
        ...
        try {
            ...
            message.loading({ content: 'Transaction confirmed! Finalizing OXYBLE unstake...', key: loadingMessageKey, duration: 0 });
            await confirmOXYBLEUnstake({
                userWalletAddress: rawAddress,
                stakeId: selectedStakeForUnstake.id,
                unstakeTransactionBoc: result.boc,
                unstakeTransactionHash: txHash
            });
            message.success({ content: "OXYBLE unstake request submitted! Backend will verify.", key: loadingMessageKey, duration: 7 });
            ...
            message.error({ content: txError?.response?.data?.message || txError?.message || 'OXYBLE unstake failed.', key: loadingMessageKey, duration: 6 });
            console.error("On-chain OXYBLE Unstake Tx Error on EarnPage:", txError);
        } finally {
        ...
        return (
            <div className="earn-page-container">
                <div style={{ display: 'flex', flexDirection: 'column',  alignItems: 'center', justifyContent: 'center', gap: 16 }}>
                    <img src="/img/staking.png" alt="staking" style={{ width: 150 }} />
                ...
                <Card className="centered-message-card">
                    ...
                    <Paragraph className="text-secondary-light" style={{ marginBottom: 24, fontSize: '1rem' }}>
                        To access OXYBLE staking plans, view your earnings, and manage your stakes, please connect your TON wallet.
                    </Paragraph>
    ...
    return (
        <Spin spinning={isLoadingInitial} tip="Loading OXYBLE Staking Hub..." size="large">
            <div className="earn-page-container">
                <Title level={2} className="page-title"><DollarCircleOutlined style={{ marginRight: 10 }} />OXYBLE Staking</Title>
                    ...
                    <Row gutter={[16, 16]} align="stretch" justify="center">
                        <Col xs={24} sm={12} className="summary-stat-col">
                            <Card className="earn-summary-card">
                                <AntdStatistic
                                    title="Your OXYBLE Wallet Balance"
                                    value={loadingBalances ? '-' : OXYBLEWalletBalance.toFixed(OXYBLE_DECIMALS)}
                                    suffix="OXYBLE"
                                />
                                {currentArxPrice != null && !loadingBalances && (
                                    <Text className="summary-value-equivalent">
                                        ~${(OXYBLEWalletBalance * currentArxPrice).toFixed(USD_DECIMALS)} USD
                                    </Text>
                                )}
                            </Card>
                        </Col>
                ...
                {userFriendlyAddress && !loadingUserStakes && userStakesData.stakes.length === 0 && isStakingAvailable && (
                    <Card className="centered-message-card" style={{ marginTop: 20 }}>
                        <Empty
                            ...
                            description={
                                <Paragraph className="text-secondary-light" style={{ fontSize: '1rem' }}>
                                    You have no active OXYBLE stakes yet. Choose a plan above to start earning USDT rewards!
                                </Paragraph>
                            }
                        />
                    </Card>
                ...
                <Modal
                    title={<Text className="modal-title-text">{`Stake OXYBLE in "${selectedPlan?.title || ''}"`}</Text>}
                    ...
                    footer={[
                        ...
                        <Button
                            ...
                            disabled={
                                !calculatedOXYBLEAmount ||
                                calculatedOXYBLEAmount <= 0 ||
                                calculatedOXYBLEAmount > OXYBLEWalletBalance ||
                        ...
                        >
                            Stake {calculatedOXYBLEAmount > 0 ? calculatedOXYBLEAmount.toFixed(OXYBLE_DECIMALS) : ''} OXYBLE
                        </Button>
                    ]}
                    width={modalWidth}
                >
                    {selectedPlan && (
                        <div className="stake-modal-content">
                            <Descriptions column={1} bordered size="small" className="modal-descriptions">
                                ...
                                <Descriptions.Item label="OXYBLE Early Unstake Penalty">
                                    <Text style={{ color: '#F44336' }}>{selectedPlan.OXYBLEEarlyUnstakePenaltyPercent}%</Text>
                                </Descriptions.Item>
                            ...
                            <Paragraph className="modal-text"><Text strong className="text-primary-light">Your OXYBLE Wallet Balance: </Text>
                                <Text style={{ fontWeight: 'bold' }} className="text-primary-light">{OXYBLEWalletBalance.toFixed(OXYBLE_DECIMALS)} OXYBLE</Text>
                            </Paragraph>
                            ...
                            {currentArxPrice && inputUsdtAmount != null && inputUsdtAmount >= 0 && (
                                <Paragraph className={`modal-calculated-OXYBLE ${calculatedOXYBLEAmount > OXYBLEWalletBalance ? 'insufficient' : ''}`}>
                                    This equals approx: <Text strong className="text-primary-light">{calculatedOXYBLEAmount.toFixed(OXYBLE_DECIMALS)} OXYBLE</Text>
                                    <Text className="price-reference"> (at ~${currentArxPrice.toFixed(4)}/OXYBLE)</Text>
                                    {calculatedOXYBLEAmount > OXYBLEWalletBalance &&
                                        <Text className="insufficient-balance-warning"> (Insufficient OXYBLE Balance)</Text>
                                    }
                            ...
                            <Alert type="info" style={{ marginTop: 16 }} className="modal-alert"
                                ...
                                description={<Text className="text-secondary-light">You are committing a specific USDT value. The system calculates the equivalent OXYBLE to be staked from your wallet. USDT rewards are based on this initial USDT value. Your OXYBLE principal is locked in the smart contract.</Text>}
                            />
                        </div>
                    )}
                </Modal>
                ...
                <Modal
                    title={<Text className="modal-title-text">Confirm OXYBLE Principal Unstake</Text>}
                    ...
                    {unstakePrepDetails && selectedStakeForUnstake ? (
                        <div className="unstake-details-modal-content">
                            ...
                            <Descriptions column={1} bordered size="small" className="modal-descriptions">
                                ...
                                <Descriptions.Item label="OXYBLE Principal Staked">{unstakePrepDetails.principalOXYBLE} OXYBLE</Descriptions.Item>
                                {unstakePrepDetails.isEarly &&
                                    <Descriptions.Item label="Early Unstake Penalty">
                                        <Text style={{ color: '#F44336' }}>{unstakePrepDetails.OXYBLEPenaltyPercentApplied}% of principal</Text>
                                    </Descriptions.Item>
                                }
                            </Descriptions>
                            <Alert
                                message={<Text className="text-primary-light">Important Note</Text>}
                                description={<Text className="text-secondary-light">This action only unstakes your OXYBLE principal from the smart contract. Your accrued USDT rewards are separate and can be withdrawn from your dashboard.</Text>}
                                type="info"
                                showIcon
                                style={{ marginTop: 16 }}
