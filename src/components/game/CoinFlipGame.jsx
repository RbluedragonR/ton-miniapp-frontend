...
const CoinflipGame = () => {
  ...
  const coinImages = {
    ...
    default: '/img/coin-default-cf.png' // A default if others fail or for initial state
  };
  const fallbackCoinImage = coinImages.default;
  ...
  const fetchArixGameBalance = useCallback(async (showMsg = false) => {
      ...
      try {
          ...
          if (walletAddr) {
              ...
              if (showMsg) message.info("ARIX wallet for Coinflip not found for this account.");
          }
      } catch (e) {
  ...
  const getCoinImageSrc = () => {
    ...
    // Default to "default" coin before a flip, or the choice image
    return choice === 'heads' ? coinImages.heads : (choice === 'tails' ? coinImages.tails : coinImages.default);
  };

  return (
    <Card className="dark-theme-card coinflip-card" bordered={false}>
      <Title level={isMobile ? 3 : 2} style={{ textAlign: 'center', color: '#ffffff', marginBottom: isMobile ? 20 : 24, fontWeight: 600 }}>
        ARIX Coinflip
      </Title>
      
      {userFriendlyAddress ? (
        <Paragraph style={{ textAlign: 'center', color: '#a0a0a5', marginBottom: isMobile ? 16 : 20, fontSize: '0.95em' }}>
            Your ARIX Balance: <Text strong style={{color: '#7e73ff', fontSize: '1.05em'}}>{arixGameBalance.toFixed(ARIX_DECIMALS)} ARIX </Text>
            <Button icon={<ReloadOutlined />} onClick={() => fetchArixGameBalance(true)} loading={balanceLoading} type="text" size="small" style={{marginLeft: 8, color: '#8e8e93'}} />
        </Paragraph>
      ) : (
          <Alert 
            message="Connect Wallet to Play" type="info" showIcon style={{ marginBottom: 20 }} className="dark-theme-alert"
            description="Please connect your TON wallet to play Coinflip with ARIX and track your game history."
            action={ <Button size="small" type="primary" onClick={() => tonConnectUI.openModal()}> Connect Wallet </Button> }
          />
      )}

      <Row gutter={[isMobile ? 16 : 24, isMobile ? 20 : 24]} justify="center" align="middle">
        <Col xs={24} md={10} style={{ textAlign: 'center', marginBottom: isMobile ? 20 : 0 }}>
          <div className="coin-display-container">
            <img src={getCoinImageSrc()} alt={flipping ? "Coin Spinning" : (gameResult?.serverCoinSide ? `Coin: ${gameResult.serverCoinSide}`: (choice || "Coin"))} 
          ...
          {gameResult && !flipping && !gameResult.error && (
            <div className={`game-result-info ${gameResult.outcome}`}>
              ...
              <Text className="result-text delta">
                ...
                <Text strong className={gameResult.outcome === 'win' ? 'game-win' : 'game-loss'}>{gameResult.outcome === 'win' ? '+' : ''}{gameResult.amountDelta.toFixed(ARIX_DECIMALS)} ARIX</Text>
              </Text>
              {gameResult.gameId && <Text className="game-id-text">Game ID: {gameResult.gameId.substring(0,10)}...</Text>}
            </div>
          )}
           {gameResult?.error && (<Alert message="Game Error" description={gameResult.message} type="error" showIcon style={{marginTop: 15}} className="dark-theme-alert"/>)}
        </Col>

        <Col xs={24} md={14} className="coinflip-controls">
          <div style={{ marginBottom: isMobile ? 16 : 20 }}>
            <Text className="input-label">Bet Amount (ARIX):</Text>
            <InputNumber style={{ width: '100%' }} className="themed-input-number" value={betAmount}
          ...
          <div style={{ marginBottom: isMobile ? 20 : 24 }}>
            ...
            <Radio.Group onChange={(e) => setChoice(e.target.value)} value={choice} buttonStyle="solid"
              disabled={flipping || !userFriendlyAddress} size="large" className="coinflip-radio-group dark-segmented-control">
          ...
          <Button type="primary" onClick={handlePlaceBet} loading={flipping}
            ...
            {flipping ? 'Flipping...' : `Flip for ${betAmount || 0} ARIX`}
          </Button>
        </Col>
      </Row>
    </Card>
