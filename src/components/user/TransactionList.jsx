    ...
    if (!items || items.length === 0) {
        return (
            <Card className="transaction-list-card empty-list-card" style={{marginTop: 0, ...listStyle}}>
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description={<Paragraph className="text-secondary-light" style={{fontSize: '1rem'}}>No {itemType || 'history'} found.{itemType === 'staking activity' && ' Explore Earn page.'}{itemType === 'Coinflip game' && ' Try Game Center.'}</Paragraph>}/>
            </Card>
        );
    }
    return (
        <Card className="transaction-list-card" title={listTitle ? <Title level={4} className="transaction-list-title">{listTitle}</Title> : null} style={{marginTop: listTitle ? 20 : 0, ...listStyle}}>
            <List
                itemLayout="vertical"
                dataSource={items}
                renderItem={(item, index) => (<List.Item key={item.id || item.game_id || item.stake_id || index} className="transaction-list-item-wrapper">{renderItemDetails(item, isMobile, itemType === 'staking activity' ? onUnstakeItemClick : undefined)}</List.Item>)}
