...
class Wallet {
    ...
    static async openFromSource(client, source) {
        let address = await (0, __1.contractAddress)(source);
    ...
    async prepare(workchain, publicKey, type = 'org.ton.wallets.v3') {
        let contra = await createContract(__classPrivateFieldGet(this, _Wallet_client, "f"), type, publicKey, workchain);
    ...
    async prepareFromSource(source) {
        let contra = await WalletContract_1.WalletContract.create(__classPrivateFieldGet(this, _Wallet_client, "f"), source);
        if (!contra.address.equals(this.address)) {
            throw Error('Contract have different address');
        }
        __classPrivateFieldSet(this, _Wallet_contract, contra, "f");
